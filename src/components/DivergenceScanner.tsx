...
function ConfirmBadge({
  ...
  continuationRate: number | null | undefined;
  followRate: number | null | undefined;
  sampleCount: number | undefined;
}) {
  if (!sampleCount || sampleCount < 5) return <span className="text-slate-600">—</span>;
...
function Sparkline({ pair, moverColor }: { pair: DivergencePair; moverColor: string }) {
  ...
  function xPos(i: number) {
    return PAD.left + (i / Math.max(n - 1, 1)) * innerW;
  }
  ...
  const moverLabel   = aIsMover ? pair.labelA : pair.labelB;
  const holdoutLabel = aIsMover ? pair.labelB : pair.labelA;
    ...
    <div className="space-y-2">
      ...
      >
        <rect
          ...
          width={W - PAD.right - dividerX} height={innerH}
          fill="rgba(255,255,255,0.03)"
        />
        <line
          x1={dividerX} y1={PAD.top} x2={dividerX} y2={PAD.top + innerH}
        ...
        />
        {[minV, 0, maxV].map((v, i) => (
          <text key={i} x={PAD.left - 6} y={yPos(v) + 4} textAnchor="end" fontSize={9} fill="#64748b">
            {v === 0 ? '0' : (v * 100).toFixed(2) + '%'}
          </text>
        ))}
        <path d={toPath(holdoutCum)} fill="none" stroke="#94a3b8" strokeWidth={1.5} opacity={0.6} />
        <path d={toPath(moverCum)} fill="none" stroke={moverColor} strokeWidth={2} />
        <circle cx={xPos(n - 1)} cy={yPos(moverCum[n - 1] ?? 0)} r={3} fill={moverColor} />
        <circle cx={xPos(n - 1)} cy={yPos(holdoutCum[n - 1] ?? 0)} r={3} fill="#94a3b8" />
        <text x={PAD.left} y={H - 4} fontSize={9} fill="#475569">−{n - 1}</text>
        <text x={dividerX} y={H - 4} fontSize={9} fill="#475569" textAnchor="middle">
          −{pair.shortWindow}
        </text>
        <text x={W - PAD.right} y={H - 4} fontSize={9} fill="#475569" textAnchor="end">now</text>
      </svg>
    </div>
  );
}
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-2">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-400">
              {pair.labelA} <span className="text-slate-600">/</span> {pair.labelB}
            </span>
          </div>
        </td>
        ...
        <td className="px-3 py-2">
          {hasClearSignal ? (
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-semibold" style={{ color: moverColor }}>{moverLabel}</span>
