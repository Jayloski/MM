 ...
 * Each pair uses only the dates where BOTH tickers have data, so one
 * illiquid ticker cannot collapse the entire matrix.
 */
export function buildCorrelationMatrix(
  ...
  for (let i = 0; i < n; i++) {
    matrix[i][i] = 1;
    const retA = returnMaps.get(tickers[i]);
    if (!retA) continue;

    for (let j = i + 1; j < n; j++) {
      const retB = returnMaps.get(tickers[j]);
      if (!retB) continue;

      const shared = Array.from(retA.keys())
        .filter(d => retB.has(d))
        .sort()
        .slice(-lookbackBars);
      ...
      const a = shared.map(d => retA.get(d)!);
      const b = shared.map(d => retB.get(d)!);
      const r = pearson(a, b);
